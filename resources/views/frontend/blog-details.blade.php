    ...
    <header class="site-header parallax-bg">
        <div class="container">
            <div class="row d-flex align-items-center">
                <div class="col-sm-8">
                    <h2 class="title">Detalhes do Blog</h2>
                </div>
                <div class="col-sm-4">
                ...
                <div class="col-sm-12">
                    ...
                    <div class="blog-meta">
                        <div class="single-meta">
                            <div class="meta-title">Publicado</div>
                            <h4 class="meta-value"><a
                                    href="javascript:void(0)">{{ \Carbon\Carbon::parse($blog->created_at)->translatedFormat('d M, Y') }}</a>
                            </h4>
                        </div>
                        <div class="single-meta">
                            <div class="meta-title">Categoria</div>
                            <h4 class="meta-value"><a href="javascript:void(0)">{{ $blog->getCategory->name }}</a></h4>
                        </div>
                    </div>
                    <figure class="img-fix">
                    ...
                    <div class="single-navigation">
                        @if ($previousPost)
                            <a href="{{ route('show.blog', $previousPost->id) }}" class="nav-link"><span class="icon"><i
                                    ...
                                    class="text">{{ Str::limit($previousPost->title, 35) }}</span></a>
                        @endif
                        @if ($nextPost)
                            <a href="{{ route('show.blog', $nextPost->id) }}" class="nav-link"><span
                                    class="text">{{ Str::limit($nextPost->title, 35) }}</span><span class="icon"><i
