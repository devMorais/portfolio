...
class FooterSocialLinkDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('icon', function ($query) {
                return '<i class="' . $query->icon . '" style="font-size:25px"></i>';
            })
            ->addColumn('action', function ($query) {
                return '<a href="' . route('admin.footer-social.edit', $query->id) . '" class="btn btn-primary btn-small"><i class="fas fa-edit"></i></a>
                <a href="' . route('admin.footer-social.destroy', $query->id) . '" class="delete-item btn btn-danger btn-small"><i class="fas fa-trash"></i></a>';
            })
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('icon')->width(300)->addClass('text-center')->title('Ícone'),
